...
Renderer.prototype.link = function(href, title, text) {
  ...
  var out = '<a '+TARGET+' href="' + escape(href) + '"';
  if (title) {
    out += ' title="' + title + '"';
  }
  out += '>' + text + '</a>';
...
function iframeView(html,toNodeEl){
  toNodeEl = toNodeEl || document.body;
  iframeMake(html, toNodeEl);
  function iframeMake(htmlFragment, toElement){
    ...
    iframe.id = 'frames';
    iframe.width = window.innerWidth-20-310;
    ...
    iframe.onload = function(){
      ...
      slink.href="../assets/style.css";
      innerDoc.head.appendChild(slink);
...
function readMD(LIarray,TOC){
  var result=[];
  for(var i=0;i<LIarray.length;i++){  
    LIarray[i].style.cursor = "pointer";
    LIarray[i].onmouseover = function(){(this).style['font-style']='italic'};
    LIarray[i].onmouseout = function(){(this).style['font-style']=''};
    var req = new XMLHttpRequest();req.open('GET',TOC[i],false);req.send(),result[i]=req.responseText;
  }
  return result
}
